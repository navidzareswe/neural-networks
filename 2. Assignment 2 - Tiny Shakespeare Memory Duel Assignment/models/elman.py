...
class ElmanRNN:
    def __init__(
        ...
        self.W_hh = rng.uniform(-0.1, 0.1, (self.hidden_size, self.hidden_size))
        ...
        self.param_names = ["W_xh", "W_hh", "W_hy", "b_h", "b_y"]
    ...
    def backward(self, one_hot, hiddens, probs, targets):
        ...
        grad_h_next = np.zeros((self.hidden_size, 1))
        total_loss = 0.0
        ...
        for t in reversed(range(seq_len)):
            ...
            delta_h  = self.W_hy.T @ out_delta + grad_h_next
            grad_h   = (1.0 - hiddens[t] ** 2) * delta_h

            grad_b_h  += grad_h
            grad_W_xh += grad_h @ one_hot[t].T
            grad_W_hh += grad_h @ hiddens[t - 1].T
            grad_h_next = self.W_hh.T @ grad_h

        grads = dict(W_xh=grad_W_xh, W_hh=grad_W_hh, W_hy=grad_W_hy, b_h=grad_b_h, b_y=grad_b_y)
    ...
    def train_step(self, inputs, targets, h_prev):
        one_hot, hiddens, probs = self.forward(inputs, h_prev)
        ...
        return loss, hiddens[len(inputs) - 1], preds

    def evaluate_step(self, inputs, targets, h_prev):
        one_hot, hiddens, probs = self.forward(inputs, h_prev)
        seq_len = len(inputs)
        loss = sum(-np.log(probs[t][targets[t], 0] + 1e-9) for t in range(seq_len))
        preds = [int(np.argmax(probs[t])) for t in range(seq_len)]
        return loss / seq_len, hiddens[seq_len - 1], preds
    ...
    def generate(
        ...
        for char_idx in seed_indices:
            ...
            x[char_idx] = 1.0
            hidden = np.tanh(self.W_xh @ x + self.W_hh @ hidden + self.b_h)
        ...
        for _ in range(length):
            ...
            x[curr_idx] = 1.0
            hidden = np.tanh(self.W_xh @ x + self.W_hh @ hidden + self.b_h)
            scaled_logits = self.W_hy @ hidden + self.b_y
            scaled_logits = scaled_logits / temp - np.max(scaled_logits / temp)
            probs = np.exp(scaled_logits) / np.sum(np.exp(scaled_logits))
            curr_idx = np.random.choice(self.vocab_size, p=probs.ravel())
    ...
    def __repr__(self):
        return (
            f"ElmanRNN(vocab={self.vocab_size}, hidden={self.hidden_size}, "
